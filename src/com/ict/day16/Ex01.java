...
/*
   List 인터페이스 : 배열과 흡사한 구조,
    ...
    add, push, addElement -> 추가 (반드시 뒤로 추가)
    add(index, E)	 	 -> 삽입 (중간에 들어감)
    pop  : 맨 위에 존재하는 객체 반환 후 (삭제)한다.
    peek : 맨 위에 존재하는 객체 반환 후 삭제를 하지 않는다.
    contains(Element e) : boolean -> 해당 Element가 포함 되어있으면 true
    search : 검색 (오른쪽 1부터) 
...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 마지막 객체 보기 : pop(삭제), peek(삭제안됨), lastElement(맨마지막 요소 반환)
		String res = s1.peek();
		System.out.println("가지고 온 객체 :" + res);
		System.out.println(s1);
		
		System.out.println();
		// 객체를 확인하고 res넣어주고 객체 삭제해준다.
		res = s1.pop();
		System.out.println("가지고 온 객체 :" + res);
		System.out.println(s1);
		...
		s1.addElement("마이콜");
		System.out.println(s1);
		System.out.println(s1.size()); // size는 index값이 아니다. 개수일뿐이다.
		...
		// 하나씩 꺼낸다. (LIFO) 
		while (! s1.isEmpty()) { // isEmpty는 비웠냐라는 뜻 
								 // 참이면 펄스이기때문에 그래서 !느낌표 트루
			String k = s1.pop();
