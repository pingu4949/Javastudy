	  ...
	  // 내부클래스 : 클래스 안에 다른 클래스가 정의 되어 있는 클래스
	  	 종류 : Member(멤버), Local(지역), static, Anonymous(익명)
	   	 1. Member
	  	   ...
	  	   특징) 내부클래스는 외부클래스의 멤버필드와 멤버 메서드를 마음대로 사용 가능
	 		    단, 상속은 privage 접근안함, 내부클래스는 private 접근가능
	 		    외부클래스를 통해서만 내부클래스를 생성할 수 있다.
	 		    (내부클래스만 별도로 객체생성이 안된다.)
	 	2. Local
	 		외부클래스{
	 		 ...
	 		 public void XXX() {
	 	...
	 	3. static 내부클래스 
	 	   외부클래스{
	  	   ...
	  	   특징) Member 내부클래스와 같은 구조를 가지고 있음
	  	    	...
	  	    	내부클래스를 static으로 만들어야 한다.
	  	    	일반 static 접근 방법을 사용하면 된다.(내부클래스이름, 변수)
	  	    	(외부클래스를 생성하지 않아도 된다.)
