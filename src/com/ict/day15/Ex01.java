...
/*
  제네릭과 컬렉션(자료구조)
  제네릭 : 컬렉션이 어떤 객체들로 이루어졌는지를 표시하는 객체 타입이다.
  컬렉션 : 객체들을 모아서 저장 관리하는 인터페이스들을 말한다.
  
  	변수 -> 하나의 데이터를 저장할 수 있는 공간
  	배열 -> 같은 종류의 자료형을 저장
  			단점 : 크기를 먼저 지정해야 된다. 변경이 안된다.
  			변수와 객체를 여러개를 저장 할 수 있다.
  	클래스 -> 배열은 같은 종류의 자료형을 저장하는 것이 불편해서 클래스의 저장하면
  	...
  	API : <T> -> 객체 타입(Type), <E> -> 요소(Element), 컬렉션안에 존재하는 객체 하나 하나를 뜻한다.
  		 ...
  		 1 : 1 대응, ** key호출하면 Value이 나온다(key 1 : Value 1)
    ...
    ** iterator() : Iterator<E>(반환형) -> 컬렉션안에 존재하는 객체들을 순서대로 접근하기 위해서 
    * 									 Iterator 객체를 반환
