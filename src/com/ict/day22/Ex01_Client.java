...
//			contentPane.setLayout(new CardLayout(0, 0));
			...
			// 첫 화면 입장하기 눌렀을때 서버에 정보를 보낸다. (닉네임, com = 1번)보냄.
			join_bt.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(name.length() > 0) {
						...
						// 서버 접속
						if(connected()) { //닉네임은 한번만 하면 넘어가는것 이기때문에 반복문이 없다.
							// 닉네임 보내기
							Ex01_Protocol p = new Ex01_Protocol();
							p.setCmd(1);
							p.setMsg(name);
							try {
								out.writeObject(p); //쓰기해서 protocol로 보냄
							} catch (IOException e1) {
								e1.printStackTrace();
							}
						}
		...
		private boolean connected() {
			boolean value = true;
			try {
				s = new Socket("192.168.0.117", 7007);
		...
		public void run() {
			...
			closed(); //위에 메서드로 올라감 프로그램 종료됨
		}
		
		
		public static void main(String[] args) {
			// AMT에서 관리하는 특별한 스레드 처리 방식
