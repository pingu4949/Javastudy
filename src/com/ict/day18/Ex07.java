...
public class Ex07 {
	public static void main(String[] args) {
		...
		// map() : String 요소들을 변경한 새로운 Stream 를 반환한다.
		//		   단일 스트림의 원소를 매핑 시킨 후 매핑 시간 값을 다시 스트림으로 변환하는 중간 연산
		//         객체에서 원하는 원소를 추출하는 역할을 한다고 볼수 있다.
		...
		List<String> list3 = stream4.flatMap(i ->{
			String s = i.substring(0, 2); //0~2글자 추출
		...
		// stream5.filter(i -> "a".equals(i[0].toString()))
		...
		stream6.flatMap(i-> Arrays.stream(i)).
